(function (global, factory) {
    ...
    Scene.prototype.add = function (object, noThree) {
        ...
        if (!noThree) THREE.Scene.prototype.add.call(this, object);
    ...
    Scene.prototype.remove = function (object) {
        if (object instanceof Constraint) {
            ...
            this.physijs.postMessage(MESSAGE_TYPES.REMOVE_CONSTRAINT, {
        ...
        if (object.physics instanceof _PhysicsObject) {
            ...
            if (object.physics.type === 'RIGID') {
                this.physijs.postMessage(MESSAGE_TYPES.REMOVE_RIGIDBODY, {
                    body_id: object.physics._.id
                });
            } else if (object.physics.type === 'GHOST') {
                this.physijs.postMessage(MESSAGE_TYPES.REMOVE_GHOSTBODY, {
                    body_id: object.physics._.id
                });
            }
        }
    ...
    function applyForce(three_body, force, local_location) {
        if (local_location == null) local_location = {
            x: 0,
            y: 0,
            z: 0
        };
    ...
    function setRigidBodyLinearVelocity(body) {
        this.physijs.postMessage(
            MESSAGE_TYPES.SET_RIGIDBODY_LINEAR_VELOCITY, {
                body_id: body.physics._.id,
                velocity: {
                    x: body.physics.linear_velocity.x,
                    y: body.physics.linear_velocity.y,
                    z: body.physics.linear_velocity.z
                }
            }
        );
    }
    ...
    function setRigidBodyAngularVelocity(body) {
        this.physijs.postMessage(
            MESSAGE_TYPES.SET_RIGIDBODY_ANGULAR_VELOCITY, {
                body_id: body.physics._.id,
                velocity: {
                    x: body.physics.angular_velocity.x,
                    y: body.physics.angular_velocity.y,
                    z: body.physics.angular_velocity.z
                }
            }
        );
    }
    ...
    function setRigidBodyLinearFactor(body) {
        this.physijs.postMessage(
            MESSAGE_TYPES.SET_RIGIDBODY_LINEAR_FACTOR, {
                body_id: body.physics._.id,
                factor: {
                    x: body.physics.linear_factor.x,
                    y: body.physics.linear_factor.y,
                    z: body.physics.linear_factor.z
                }
            }
        );
    }
    ...
    function setRigidBodyAngularFactor(body) {
        this.physijs.postMessage(
            MESSAGE_TYPES.SET_RIGIDBODY_ANGULAR_FACTOR, {
                body_id: body.physics._.id,
                factor: {
                    x: body.physics.angular_factor.x,
                    y: body.physics.angular_factor.y,
                    z: body.physics.angular_factor.z
                }
            }
        );
    }
